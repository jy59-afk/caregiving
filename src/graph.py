...
def build_graph():
    ...
    # --- nodes -----------------------------------------------------------
    builder.add_node("intake", run_intake)  # transcript -> profile
    builder.add_node("match_rank", run_match_and_rank)  # profile -> ranked shortlist (calls search_services)
    builder.add_node("explain_draft", run_explain_and_draft)  # top match -> draft message (no send)
    builder.add_node("consent_gate", run_consent_gate)  # HITL checkpoint (no side effect)
    builder.add_node("output", run_output)  # decide outcome + write final_response
