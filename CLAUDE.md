...
7. ✅ Interface — `src/app.py` (Streamlit chat over `graph.run`): node-transition logging, index pre-flight, `matches_ready` surfaces the shortlist + grounding-passage peek + a copy-friendly (never-sent) draft block, `needs_clarification` shows the question inline. Run: `streamlit run src/app.py` (or via `.claude/launch.json`). Slice-7 fix: bumped `intake` `max_tokens` 512→1536 (gpt-oss burns hidden reasoning tokens; terse input was tripping a Groq JSON-validation 400).
8. ✅ Evaluation harness — `src/evaluate.py` (8 scripted caregiver scenarios through `graph.run`; wraps `llm.complete` + `search_services` with counting shims) reports schema-validation pass rate, tool-call success rate, task-completion rate, an answer-fidelity proxy (+ optional `--judge` LLM pass) and recall@3, and writes a slide-ready table to `docs/EVALUATION.md`. `tests/test_eval.py` runs the suite and asserts the guardrail rates, double-gated behind `RUN_EVAL=1` + a built index so `pytest` stays offline. Last run: schema 100%, tool 100%, task-completion 8/8, 0 fidelity issues, recall@3 0.80.
9. ✅ Polish — README rewritten (finished-build quickstart + guardrail story + trace sample); node-transition tracing moved into `graph._traced` (wraps every node, so `python src/graph.py`, the Streamlit UI and the eval harness all emit an identical `-> node / <- node / route: / outcome:` trace — noisy libs muted, Windows stdout forced to UTF-8); `src/app.py` `run_pipeline` slimmed to a thin bookend around `graph.run()`; `docs/DEMO_SCRIPT.md` added (≤3-min video walkthrough mapped to the judging criteria). `pytest -q` = 55 green, 1 skipped; eval re-run 100%/100%/8-of-8/recall 0.80.
